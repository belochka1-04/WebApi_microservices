...
namespace WebApi_MaskService.Services
{
    public class DatabaseService : IDatabaseService, IDatabaseHealthCheck
    {
        ...
        public DatabaseService(KameraDbContext dbContext, IJobClient jobClient)
        {
        ...
        public async Task<IEnumerable<Mask>> GetAllMasksAsync(int jobId)
        {
            try
            {
                var job = await _jobClient.GetJobByIdAsync(jobId);

                if (job == null || job.UserId == null || job.UserId == 0)
                    return Enumerable.Empty<Mask>();

                var masks = await _dbContext.Masks
                    .Where(m => m.UserId == job.UserId.Value)
                    .ToListAsync();
            ...
            {
                _logger.Error(ex, "Ошибка при получении масок");
                return Enumerable.Empty<Mask>();
            }
        }
